...
function scrapeEvent() {
	var raw_details = document.getElementsByClassName("_4n-j _3cht fsl");
	if(raw_details[0] == undefined) {
		document.querySelectorAll('[data-active-tab="about"]')[0].click();
		setTimeout(function() {scrapeEvent(); return;}, 3000);
	} else {
		if (document.getElementsByClassName("see_more_link")[0] != undefined) {
		...
		if (raw_location_name[0] == undefined) {
			raw_location_name = document.getElementById("u_0_1a").innerHTML;
		} else {
			raw_location_name = raw_location_name[0].innerHTML;
			raw_location_address = document.getElementsByClassName("_5xhp fsm fwn fcg")[1].innerHTML;
		}
		var event_name = document.querySelectorAll('[data-testid=event-permalink-event-name')[0].innerHTML;
		raw_details = raw_details[0].innerText;
		...
		url += "&details=" + encodeURIComponent(raw_details + window.location.href + "\n\nGenerated by Facebook Event Puller");
		url += "&text=" + encodeURIComponent(event_name);
		url += "&location=" + encodeURIComponent(raw_location_name);
		if (raw_location_address != "") {
			 url += ", " + raw_location_address;
		}
